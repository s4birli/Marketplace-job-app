            ...
            <ScrollView>
                <View style={styles.secondModal}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }} >
                        <TouchableOpacity style={styles.confirm} onPress={() => { sendCustomOffer(con_id, token, offerDetails, selectedDays, selectedOffer.r_id, price) }}>
                            <Text style={{ color: 'white' }}>SEND</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.cancel, { marginLeft: 10 }]} onPress={() => cancelButton()}>
...
class ChatScreen extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            messages: [],
            isTyping: false,
            conversation_id: this.props.navigation.state.params.user.conversation_id,
            timeZone: this.props.navigation.state.params.user.timezone,
            ...
            blocked: this.props.navigation.state.params.user.blocked_by && this.props.navigation.state.params.user.blocked_by.some(id => id == this.props.id) ? true : false,
            reportAbuseAlert: false,
            violation: false,
            addedToFavourites: this.props.navigation.state.params.user.favourite_by && this.props.navigation.state.params.user.favourite_by.some(id => id == this.props.id) ? true : false,
            activeRootCount: 0,
            userLogOut: false,
            userOnline: this.props.navigation.state.params.user.is_online_class == 'green' ? true : false,
    ...
    reportAbuse = async () => {
        ...
        if (reportAbuseAlert) {
            let res = await blockUserService(this.props.navigation.state.params.user.id, 1, this.props.token)
            if (res.status == 1) {
    ...
    sendCustomOffer = async (con_id, token, offer_details, selectedDays, r_id, offerPrice) => {
        if (!selectedDays) {
            ...
            let customOfferArray = this.state.messages;
    ...
    showCustomOfferRoots = () => {
        if (this.state.activeRootCount != 0) {
            ...
            return (
                <View>
                    <Text style={{ fontSize: 15, color: 'red' }}>You must have atleast one active root</Text>
                </View>
            )
        }
    }
        ...
        return (
            <KeyboardAvoidingView 
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', }}>
                        ...
                        <View
                            ...
                            <View style={{ flexDirection: 'row', paddingTop: 10, justifyContent: 'center', alignItems: 'center' }}>
                                ...
                                {this.props.navigation.state.params.user.rating_count == 0 ? null :
                                    <>
                                        <Rating
                                            imageSize={18}
                                            readonly
                                            ...
                                            style={{ marginLeft: 10 }}
                                        />
                                        <Text style={{ color: '#f1c40f', fontSize: 15 }}>{this.props.navigation.state.params.user.rating}</Text>
                                        <Text style={{ color: 'black', fontSize: 15, marginLeft: 5 }}>{'(' + this.props.navigation.state.params.user.rating_count + ')'}</Text>
                                    </>}
                            </View>
                            <View style={{ flexDirection: 'row' }}>
                ...
                <View>
                    ...
                    {
                        this.state.isSecondModalVisible ?
                            <ShowCustomOfferRootsDetails
                                item={this.state.selectedOffer}
                                show={this.state.isSecondModalVisible}
                                con_id={this.state.conversation_id}
                                token={this.props.token}
                                selectedOffer={this.state.selectedOffer}
                                offerDetails={this.state.offer_details}
                                selectedDays={this.state.selectedDaysForOffer}
                                price={this.state.offerPrice}
                                cancelButton={() => this.setState({ isSecondModalVisible: false })}
                                sendCustomOffer={this.sendCustomOffer}
                                setOfferDetail={this.setOfferDetail}
